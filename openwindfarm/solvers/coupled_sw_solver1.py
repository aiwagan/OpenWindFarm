...
class CoupledSWSolverParameters1(FrozenClass):
    """ A set of parameters for a :class:`CoupledSWSolver1`.
...
class CoupledSWSolver1(Solver):
    ...
    :class:`CoupledSWSolverParameters1`.
    """
    ...
    def __init__(self, problem, solver_params):
        ...
        if not isinstance(solver_params, CoupledSWSolverParameters1):
            raise TypeError, "solver_params must be of type \
CoupledSWSolverParameters1."
    ...
    def default_parameters():
        """ Return the default parameters for the :class:`CoupledSWSolver1`.
        """
        return CoupledSWSolverParameters1()
    ...
    def solve(self, annotate=True):
        ...
        for function_name, u_expr, facet_id, bctype in problem_params.bcs:
            ...
            if bctype == 'weak_dirichlet':
                ...
                bc_contr -= -dot(u_mid, n) * q * ds(facet_id)
                bc_contr -=  dot(u_expr, n) * q * ds(facet_id)
        ...
        for function_name, eta_expr, facet_id, bctype in weak_eta_bcs:
            ...
            C_mid +=  inner(dot(v, n), (eta_expr-h_mid)) * ds(facet_id)
        ...
        R_mid = friction * norm_u_mid * inner(u_mid, v) * dx(self.mesh)

        if farm:
            R_mid += tf*dot(u_mid, u_mid)**0.5*inner(u_mid, v)*farm.site_dx
